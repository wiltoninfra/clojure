...
The `home-routes` handler in the `codeview.routes.home` namespace
...
The `home-page` function will in turn call the `codeview.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `codeview.handler` namespace:
...
Request middleware functions are located under the `codeview.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `codeview.dev-middleware` namespace found in
